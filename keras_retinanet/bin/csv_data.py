...
def get_data(input_path):
	...
	'imageset': 'trainval','width': 486, 'filepath':'F:/study_files/faster_rcnn/training_data/VOCdevkit\\VOC2012\\JPEGImages\\2007_000027.jpg',
	...
	for data_path in data_paths:
		...
		test_files = []
		try:
			with open(imgsets_path_trainval) as f:  # 打开文件，默认位只读模式
				"""得到训练集图片文件的名称"""
				for line in f:  # 按行读取对象类的文字
					trainval_files.append(line.strip() + '.jpg')  # 去除空格
		...
		for annot in annots:
			...
			try:
				...
				element_filename = element.find('filename').text  # 得到图片的名称
...
def annotation_csv(input_path, output_path):
	...
	output_file = os.path.join(output_path, 'annotation.csv')
...
annotation_csv('D:/jupyter_file/keras_frcnn_git/VOCdevkit', './out')
mapping_csv('D:/jupyter_file/keras_frcnn_git/VOCdevkit', './out')